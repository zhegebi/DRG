...
from .models import MedicalRecord, Complication, DrgResult, NAME_TO_CODE, DIAG_TO_MDC, MDC_ADRG_DRG, PROCEDURE_TO_ADRG, MCC_AND_CC
...
def extract_medical_record_info(medical_record_text: str) -> MedicalRecord:
    logger.info("extracting medical record info from text")
    # 1. call api
    try:
...
"""
def get_mdc_code(medical_record_text:str, medical_record: MedicalRecord) -> str:
    logger.info("getting MDC code from medical record")
...
def get_adrg_code(medical_record: MedicalRecord, mdc_code: str) -> str:
    logger.info("getting ADRG code from medical record")
...
def get_mcc_cc_level(
    medical_record: MedicalRecord
) -> Literal[Complication.CC, Complication.MCC, Complication.NO]:
    logger.info("getting MCC and CC level from medical record")
    # 1. get primary diagnosis code
    primary_diagnosis_code = medical_record.primary_diagnosis.code
    if primary_diagnosis_code is None:
        primary_diagnosis_code = NAME_TO_CODE.diagnosis.get(medical_record.primary_diagnosis.name, None)
...
def get_drg_code_and_name( 
    ...
    """
    logger.info("getting DRG code from medical record")
    # 1. get DRG code list
