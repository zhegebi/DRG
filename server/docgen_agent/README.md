...
```bash
curl -X POST http://localhost:8000/api/docgen_agent/generate-doc \
...
```bash
GET  /api/docgen_agent/runs/{run_id}/trace       # 轮询进度
POST /api/docgen_agent/runs/{run_id}/hint        # 运行中追加提示
POST /api/docgen_agent/runs/{run_id}/interrupt    # 协作式中断
POST /api/docgen_agent/runs/{run_id}/terminate    # 协作式终止
GET  /api/docgen_agent/runs/{run_id}/download     # 下载 MD
GET  /api/docgen_agent/runs/{run_id}/download/pdf # 下载 PDF
